...
- Exception: Homebrew taps in `nix-darwin/flake.nix` (`bendews/tap`, `tursodatabase/tap`, `libsql/sqld`, `ariga/tap`) cannot be pinned to a content hash — Homebrew has no such mechanism for taps — and `trusted = true` on each is not optional: Homebrew 5.1+ refuses to install third-party-tap formulae without it (confirmed live in commit `ad07cb8`, "fix(nix-darwin): make homebrew config compatible with Homebrew 5.1"). Renewal mechanism: re-justify each tap's necessity whenever the tap list changes, and at least quarterly regardless (tsukasaI/dotfiles#25).
