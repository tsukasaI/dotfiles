...
- Don't silently drop errors from fallible iterators — handle each `Err` or collect failures alongside successes. Also watch `if let Ok(x) = ...`, which discards the `Err` arm the same way.
